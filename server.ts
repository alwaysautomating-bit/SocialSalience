...
// Hardcoded starter mock data in case Airtable has no records or is not configured yet.
// This ensures "Default state = usable immediately" (open -> use -> done philosophy).
...
// Helper to check standard Airtable keys
...
// Field ID resolution: env vars first, hardcoded defaults as fallback.
// Query must include returnFieldsByFieldId=true for these IDs to match.
const SIGNAL_FIELDS = {
...
// Normalize a raw Airtable record into a clean SocialSignal shape.
// Tries field ID first (when returnFieldsByFieldId=true), then common display-name fallbacks.
...
app.get("/api/signals", async (req, res) => {
  ...
  if (!config.isConfigured) {
    console.log("Airtable API Key not configured. Returning premium sandbox signals.");
    return res.json({
      success: true,
      mode: "sandbox",
      signals: SANDBOX_SOCIAL_SIGNALS
    });
  }

  try {
    // returnFieldsByFieldId=true makes Airtable key fields by ID instead of display name,
    // which is required for our SIGNAL_FIELDS ID-based lookups to work.
    ...
    if (!response.ok) {
      ...
      throw new Error(`Airtable API response error: ${response.status} - ${errText}`);
    }

    const data = await response.json();

    console.log(`[Airtable] Fetched ${data.records?.length ?? 0} signal records.`);
    if (data.records?.length > 0) {
      const sample = data.records[0];
      console.log(`[Airtable] First record id=${sample.id}, field keys:`, Object.keys(sample.fields || {}));
    }

    const signals = data.records.map((rec: any) => {
      const normalized = normalizeSignalRecord(rec);
      if (!normalized.signal) {
        console.warn(`[Airtable] Record ${rec.id} has empty signal text. Raw fields:`, rec.fields);
      }
      return normalized;
    });
    ...
    res.status(500).json({
      ...
      mode: "sandbox",
      signals: SANDBOX_SOCIAL_SIGNALS,
      note: "Felled back to sandbox signals due to connection failure."
    });
  }
});

// 2. POST /api/suggest-connections
app.post("/api/suggest-connections", async (req, res) => {
  const { signal, insights } = req.body;

  if (!signal) {
    return res.status(400).json({ success: false, error: "External signal text is required" });
  }
  ...
  if (!geminiKey || geminiKey.trim() === "" || geminiKey === "MY_GEMINI_API_KEY") {
    return res.status(400).json({
      success: false,
      error: "GEMINI_API_KEY is not configured. Please add it to your Secrets in settings."
    });
  }
...
Do not write a full social post yet.`;
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              connection_name: {
                type: Type.STRING,
                description: "Short headline of the connection"
              },
              matched_insight_title: {
                type: Type.STRING,
                description: "The exact internal insight text that this connects to"
              },
              connection_type: {
                type: Type.STRING,
                description: "Must be exactly one of: Validates, Contradicts, Extends, Challenges, or Names"
              },
              why_it_connects: {
                type: Type.STRING,
                description: "Why this external signal highlights or prompts the internal insight"
              },
              gap_opportunity: {
                type: Type.STRING,
                description: "The market gap, user friction, or structural opportunity revealed by this collision"
              },
              suggested_angle: {
                type: Type.STRING,
                description: "The conceptual or operational angle of our response to this signal"
              },
              strength_score: {
                type: Type.INTEGER,
                description: "Numeric strength of the connection, from 1 to 5 stars"
              },
              draft_brief: {
                type: Type.STRING,
                description: "A bulleted or brief outline/brief to create a draft post later"
              }
            },
            required: [
              "connection_name",
    ...
    try {
      ...
      if (!Array.isArray(suggestions)) {
        console.warn("[Gemini] Response parsed but is not an array. Wrapping:", typeof suggestions);
        suggestions = [suggestions];
      }
    } catch (parseErr) {
      console.error("[Gemini] JSON parse failed. Raw text (first 500 chars):", rawText.substring(0, 500));
      throw new Error("Gemini returned a non-JSON response. Check model availability and prompt.");
    }
    ...
    console.error("[Gemini] Connection suggestion error:", err);
    res.status(500).json({
      success: false,
      error: "Gemini execution failed: " + err.message
    });
  }
});
...
app.post("/api/connections", async (req, res) => {
  ...
  if (!connection) {
    return res.status(400).json({ success: false, error: "Connection object is required" });
  }

  const config = getAirtableConfig();

  // If we are in sandbox mode, simulate a successful response
  if (!config.isConfigured) {
    console.log("Simulating connection approval in Sandbox mode.");
    return res.json({
      success: true,
      mode: "sandbox",
      message: "Approved successfully in Sandbox simulation mode. (Airtable API Key not configured)",
  ...
  try {
    ...
    // Link external signal only when it's a real Airtable record ID.
    ...
    console.log("[Airtable] Posting connection fields:", airtableFields);
    ...
    console.error("Error creating record in Airtable Connections:", error);
    res.status(500).json({
      success: false,
      error: "Failed to write approved connection to Airtable: " + error.message
    });
  }
});
...
app.get("/api/audiences", async (req, res) => {
  ...
  try {
    ...
    const data = await response.json();
    console.log(`[Airtable] Fetched ${data.records?.length ?? 0} audience records.`);
    ...
    if (audiences.length === 0) {
      console.log("[Airtable] No audiences found — falling back to sandbox.");
      return res.json({ success: true, mode: "sandbox", audiences: SANDBOX_AUDIENCES });
    }
...
app.post("/api/generate-drafts", async (req, res) => {
  ...
  if (!Array.isArray(audiences) || audiences.length === 0) {
    return res.status(400).json({ success: false, error: "At least one audience is required" });
  }
...
Return an array of ${audiences.length * 2} post draft objects.`;
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              audience_id:           { type: Type.STRING, description: "The audience ID from the list above" },
              audience_name:         { type: Type.STRING, description: "The audience name" },
              platform:              { type: Type.STRING, description: "LinkedIn, X (Twitter), or Newsletter" },
              hook:                  { type: Type.STRING, description: "The opening 1-2 sentences" },
              post_draft:            { type: Type.STRING, description: "Full post body text" },
              cta:                   { type: Type.STRING, description: "Call to action line" },
              why_this_angle_works:  { type: Type.STRING, description: "1-2 sentences on why this framing fits this audience" }
            },
    ...
    try {
      ...
      console.error("[Gemini] Draft JSON parse failed:", rawText.substring(0, 500));
      throw new Error("Gemini returned a non-JSON response for draft generation.");
    }

    console.log(`[Gemini] Generated ${drafts.length} drafts for ${audiences.length} audiences.`);
...
app.post("/api/post-drafts", async (req, res) => {
  ...
  if (!config.isConfigured) {
    ...
    return res.json({
      success: true,
      mode: "sandbox",
      recordId: "recDraft" + Math.random().toString(36).substring(7),
      postId: sandboxPostId,
      message: "Draft saved in sandbox simulation mode."
    });
  }
  ...
  try {
    ...
    if (!isSandboxConnection) {
      fields[POST_FIELDS.connection] = [connectionRecordId];
    }
    if (!isSandboxAudience) {
      fields[POST_FIELDS.audience] = [audienceRecordId];
    }
    if (batchName) {
      fields[POST_FIELDS.batchName] = batchName;
    }
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Social Salience server running on http://localhost:${PORT}`);
  });
}
